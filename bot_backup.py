...
def send_welcome(message):
    ...
    configured_chat_ids = [id.strip()
                           for id in credentials.chat_ids.split(",")]
    ...
    if not available_photos:
        ...
        bot.send_message(
            chat_id, "Извините, не удалось найти фотографии из-за проблем с соединением или их отсутствия.")
        return
    ...
    while attempts < max_attempts:
        ...
        try:
            ...
            if photo is None:
                available_photos.remove(
                    photo) if photo in available_photos else None
                continue
            ...
            if photo.photoslice_time is None:
                ...
                if (photo_date.day == today.day and
                    ...
                    comment = "Это %s. Дело было в этот день в %s году" % (
                        photo_path_splited[len(photo_path_splited) - 2], photo.photoslice_time.year)
                else:
                    comment = "Это %s. Дело было в %s %s года" % (photo_path_splited[len(
                        photo_path_splited) - 2], numberToMonthNameRu(photo.photoslice_time.month), photo.photoslice_time.year)
                ...
                if photo.name.lower().endswith('.heic'):
                    ...
                    try:
                        ...
                        with Image.open(dst + photo.name) as my_image:
                            ...
                            bot.send_photo(chat_id, open(
                                dst + 'compressed.jpg', 'rb'), caption=comment)
                        ...
                        if photo.name.lower().endswith('.heic'):
                            ...
                            try:
                                ...
                                with Image.open(dst + photo.name) as heic_image:
                                    ...
                                    # Сохраняем как JPEG
                                    validated_img.save(
                                        dst + 'heic_converted.jpg', 'JPEG', quality=85, optimize=True)
                                    bot.send_photo(chat_id, open(
                                        dst + 'heic_converted.jpg', 'rb'), caption=comment)
                                ...
                                bot.send_message(
                                    chat_id, f"Ошибка обработки HEIC: {comment}")
                        else:
                            ...
                            if validate_and_fix_image(dst + photo.name, dst + 'validated.jpg'):
                                ...
                                bot.send_photo(chat_id, open(
                                    dst + 'validated.jpg', 'rb'), caption=comment)
            ...
            if photo.media_type == "video":
                if "gp3" in photo.name or "mp4" in photo.name or "avi" in photo.name:
                    ...
                    if not downloadFile(photo.file, photo.name):
                        ...
                        bot.send_message(
                            chat_id,
                            f"Не удалось скачать видео: {comment}"
                        )
                        return
                    ...
                    if not os.path.exists(dst + photo.name):
                        ...
                        bot.send_message(
                            chat_id, f"Видео повреждено при загрузке: {comment}")
                        return

                    try:
                        bot.send_video(chat_id, open(
                            dst + photo.name, 'rb'), caption=comment)
                        os.remove(dst + photo.name)
...
def process_image_with_validation(my_image, memorySizeRatio):
    ...
    if image_width > max_dimension or image_height > max_dimension:
        scale_factor = min(max_dimension / image_width,
    ...
    # Сжимаем изображение
    my_image = my_image.resize((int(image_width / (2 * memorySizeRatio)),
                               int(image_height / (2 * memorySizeRatio))), PIL.Image.LANCZOS)
        ...
        if image_path.lower().endswith('.heic'):
            ...
            try:
                ...
                with Image.open(image_path) as img:
                    ...
                    # Сохраняем как JPEG
                    validated_img.save(output_path, 'JPEG',
                                       quality=85, optimize=True)
...
def apply_size_validation(img):
    ...
    if width > MAX_DIMENSION or height > MAX_DIMENSION:
        ...
        needs_resize = True
        print(
            f"Превышены максимальные размеры, масштабируем к: {new_width}x{new_height}")
    ...
    aspect_ratio = max(new_width / new_height, new_height /
                       new_width) if new_height > 0 else 1
    if aspect_ratio > MAX_ASPECT_RATIO:
        ...
        needs_resize = True
        print(
            f"Некорректное соотношение сторон, корректируем к: {new_width}x{new_height}")
...
def recievingFile(file, message):
    uploadedFileName = message.caption if message.caption != None else file.split(
        '/')[-1]
...
def dump_prin():
    ...
    print("Sending random photo. Next will be send at " +
          next_in.strftime("%d/%m/%Y %H:%M:%S") + " after " + str(skip_time) + " hours")
...
def schedule_random_photo():
    ...
    next_in = now + timedelta(minutes=skip_time)
    print("Schedulling. Next will be send at " +
          next_in.strftime("%d/%m/%Y %H:%M:%S") + " after " + str(skip_time) + " hours")
    schedule.enter(skip_time * 3600, 1, dump_prin, ())
    schedule.run()
